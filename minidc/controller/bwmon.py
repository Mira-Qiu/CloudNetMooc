...
class BandwidthMonitor(app_manager.RyuApp):
    ...
    def statsReplyHandler(self, ev):
        ...
        # [ ADD YOUR CODE HERE ]
	for stats in body:
		if stats.port_no in self.topo.ports[name].keys():
			hostname = self.topo.ports[name][stats.port_no]
			if hostname in self.topo.hosts:
				self.bwstats.addHostBwStat(hostname, stats.tx_bytes, stats.rx_bytes)
